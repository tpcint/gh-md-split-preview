...
````` ```mermaid ````` 블록은 marked가 코드로 남기므로, 렌더가 끝난 뒤 [mermaid](https://github.com/mermaid-js/mermaid)로 SVG를 그려 그 자리에 끼워 넣습니다. 다이어그램만큼 높이가 늘어나니 그 시점에 스크롤 앵커를 다시 잡습니다. mermaid도 marked와 같이 `@require`로 받으므로(설치할 때 한 번, 이후 Tampermonkey 캐시) 실행 중 네트워크 요청은 여전히 없습니다. 다만 번들이 3.5MB라 diff에 md가 없는 화면에서도 파싱 비용(≈40ms)은 들어갑니다.
...
- math(`$$…$$`) 등 나머지 GitHub 전용 위젯은 여전히 코드블록 그대로 나옵니다.
